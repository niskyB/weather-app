...
export const Weather = () => {
  ...
  const { data: weather, isFetching, error } = useGetWeather(debounceValue);
  ...
  return (
    <div className="max-w-screen-xl min-h-screen mx-auto px-4 lg:px-6 flex flex-col items-center py-10">
      <Input value={value} onChangeInput={onChangeInput} error={error} />
      <div className="max-h-screen w-full flex items-center justify-center mt-7">
        <div className="flex flex-col bg-white rounded p-4 w-full max-w-xs min-h-96">
          {isFetching ? (
            <>
                  ...
                  <div>
                    ...
                    <span className="text-sm font-light text-gray-500">
                      {weather?.main.temp_min
                        ? Math.ceil(weather?.main.temp_min)
                        : "-"}
                      °C
                    </span>
                  </div>
